...
export function VirtualizedExamList({ 
  exams, 
  isLoading, 
  onSelectExam,
  itemHeight = 64,
  containerHeight = 400
}: VirtualizedExamListProps) {
  ...
  // Adjust container height based on how many items we have
  const adjustedContainerHeight = useMemo(() => {
    return Math.min(containerHeight, exams.length * itemHeight + 50); // 50px for header
  }, [containerHeight, exams.length, itemHeight]);
    ...
    <div className="px-4 py-5 sm:p-6">
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          >
            {/* Spacer div to maintain total height */}
